import type { Metadata } from "next";
import Navbar from "@/components/layout/Navbar";
import Footer from "@/components/layout/Footer";
import AuditoriaForm from "@/app/diagnostico-en-vivo/AuditoriaForm";

export const metadata: Metadata = {
  title: "Agencia LinkedIn Ads LATAM — SaaS B2B & Enterprise | JT Ads",
  description:
    "Gestión profesional de LinkedIn Ads para SaaS B2B y empresas enterprise en LATAM. Lead Gen Forms, Sponsored Content, ABM y tracking conectado al CRM. Diagnóstico gratuito.",
  alternates: {
    canonical: "https://jtads.com/agencia-linkedin-ads-latam",
    languages: {
      "es": "https://jtads.com/agencia-linkedin-ads-latam",
      "x-default": "https://jtads.com/agencia-linkedin-ads-latam",
    },
  },
  keywords: [
    "agencia linkedin ads latam",
    "linkedin ads saas b2b latam",
    "linkedin ads para empresas",
    "linkedin ads mexico",
    "linkedin ads colombia",
    "publicidad linkedin latam",
  ],
  openGraph: {
    title: "Agencia LinkedIn Ads LATAM — SaaS B2B & Enterprise | JT Ads",
    description:
      "Gestión profesional de LinkedIn Ads para SaaS B2B y enterprise en LATAM. Leads de calidad con tracking conectado al CRM.",
    images: ["/og-image.png"],
    url: "https://jtads.com/agencia-linkedin-ads-latam",
  },
  twitter: { card: "summary_large_image" },
};

const services = [
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9a2 2 0 00-2-2h-2m-4-3H9M7 16h6M7 8h6v4H7V8z" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
      </svg>
    ),
    title: "Sponsored Content",
    body: "Contenido patrocinado en el feed de LinkedIn para decision-makers. Segmentación por cargo, industria, tamaño de empresa y seniority. El formato correcto para construir pipeline en ciclos de venta largos.",
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
      </svg>
    ),
    title: "Lead Gen Forms",
    body: "Formularios nativos de LinkedIn con precarga de datos del perfil profesional. CPL más bajo que landing externo, mayor tasa de completado y leads con información verificada de empresa y cargo.",
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
      </svg>
    ),
    title: "Message Ads & Conversation Ads",
    body: "Mensajes directos a la bandeja de entrada de prospectos calificados. Alta tasa de apertura para invitaciones a demos, webinars o contenido de alto valor. Ideal para ABM y outreach a cuentas target.",
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
      </svg>
    ),
    title: "Account-Based Marketing (ABM)",
    body: "Campañas dirigidas a listas de cuentas específicas — CRM upload o Company lists. Coordinamos el targeting con tu equipo de ventas para sincronizar mensajes con el ciclo del deal.",
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
      </svg>
    ),
    title: "Video & Document Ads",
    body: "Thought leadership en formato video y documentos descargables para nutrir prospectos en etapa de consideración. Ideales para posicionar expertise antes de la conversación de ventas.",
  },
  {
    icon: (
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0a2 2 0 002 2h2a2 2 0 002-2m0 0V5a2 2 0 012-2h2a2 2 0 012 2v14a2 2 0 01-2 2h-2a2 2 0 01-2-2z" strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} />
      </svg>
    ),
    title: "Tracking & CRM Attribution",
    body: "Conectamos las conversiones de LinkedIn Insight Tag con tu CRM para cerrar el loop entre inversión publicitaria y revenue real. Sabemos qué campañas generan MQLs que cierran, no solo leads.",
  },
];

const faqItems = [
  {
    q: "¿LinkedIn Ads funciona para empresas en LATAM?",
    a: "Sí, especialmente para SaaS B2B, consultoría enterprise, servicios financieros y educación ejecutiva. La penetración de LinkedIn en LATAM ha crecido significativamente — México, Colombia y Argentina tienen volúmenes suficientes para campañas B2B efectivas. El CPL es más alto que Meta, pero la calidad del lead es notablemente mejor para ciclos de venta complejos.",
  },
  {
    q: "¿Cuál es el presupuesto mínimo recomendado para LinkedIn Ads?",
    a: "Recomendamos mínimo $5,000 USD/mes en inversión en plataforma. LinkedIn tiene costos por clic y CPM más altos que otras plataformas — por eso el volumen importa para que el algoritmo aprenda correctamente. Con presupuestos menores, el volumen de datos es insuficiente para optimizar.",
  },
  {
    q: "¿Cómo miden el ROI de LinkedIn Ads más allá del CPL?",
    a: "Conectamos LinkedIn Insight Tag con tu CRM (HubSpot, Salesforce, Pipedrive) para rastrear qué leads progresan en el pipeline y cuáles cierran. Eso nos permite optimizar campañas por calidad de lead, no solo por volumen. El CPL de plataforma es el punto de partida, no el objetivo final.",
  },
  {
    q: "¿Trabajan solo con LinkedIn o también hacen mix con otras plataformas?",
    a: "Dependiendo de tu ciclo de venta y ICP, combinamos LinkedIn con Google Ads (Search de intención) y Meta Ads (retargeting). LinkedIn es excelente para prospecting de decision-makers, pero complementado con Google captura la intención cuando están buscando activamente una solución.",
  },
];

const faqSchema = {
  "@context": "https://schema.org",
  "@type": "FAQPage",
  mainEntity: faqItems.map((item) => ({
    "@type": "Question",
    name: item.q,
    acceptedAnswer: { "@type": "Answer", text: item.a },
  })),
};

const breadcrumbSchema = {
  "@context": "https://schema.org",
  "@type": "BreadcrumbList",
  itemListElement: [
    { "@type": "ListItem", position: 1, name: "Inicio", item: "https://jtads.com" },
    { "@type": "ListItem", position: 2, name: "Agencia LinkedIn Ads LATAM", item: "https://jtads.com/agencia-linkedin-ads-latam" },
  ],
};

export default function AgenciaLinkedInAdsPage() {
  return (
    <>
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(faqSchema) }}
      />
      <script
        type="application/ld+json"
        dangerouslySetInnerHTML={{ __html: JSON.stringify(breadcrumbSchema) }}
      />
      <Navbar />
      <main className="pt-0 bg-white text-slate-900 antialiased">

        {/* ── HERO ── */}
        <section className="relative py-20 lg:py-32 overflow-hidden bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-12 items-center">

              {/* Left: copy */}
              <div>
                <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#0A66C2]/10 text-[#0A66C2] rounded-full text-xs font-semibold uppercase tracking-widest mb-6">
                  SaaS B2B · Enterprise · Lead Generation
                </div>
                <h1 className="text-4xl lg:text-5xl font-extrabold text-slate-900 leading-tight mb-6">
                  Agencia de LinkedIn Ads para SaaS B2B y enterprise en LATAM
                </h1>
                <p className="text-lg text-slate-600 mb-8 leading-relaxed">
                  Generación de leads B2B calificados con LinkedIn Ads. Segmentación por cargo
                  e industria, Lead Gen Forms nativos y tracking conectado a tu CRM — para saber
                  exactamente qué campañas generan revenue, no solo leads.
                </p>
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center mb-4">
                  <a
                    href="#diagnostico-form"
                    className="bg-[#0A66C2] text-white px-8 py-4 rounded-lg font-bold text-lg hover:bg-[#0853a0] hover:shadow-lg hover:shadow-blue-200 transition-all"
                  >
                    Solicitar diagnóstico gratuito
                  </a>
                </div>
                <div className="text-sm text-slate-500 space-y-1">
                  <p>• Sin llamada de ventas previa</p>
                  <p>• Respuesta en menos de 4 horas hábiles</p>
                </div>
              </div>

              {/* Right: LinkedIn dashboard mockup */}
              <div className="relative">
                <div className="bg-slate-100 rounded-2xl p-4 shadow-2xl border border-slate-200">
                  <div className="bg-white rounded-xl shadow-sm overflow-hidden border border-slate-100">
                    {/* Browser chrome */}
                    <div className="h-8 bg-slate-50 border-b border-slate-100 flex items-center px-4 gap-1.5">
                      <div className="w-2.5 h-2.5 rounded-full bg-red-400" />
                      <div className="w-2.5 h-2.5 rounded-full bg-yellow-400" />
                      <div className="w-2.5 h-2.5 rounded-full bg-green-400" />
                      <div className="flex-1 mx-4 h-4 bg-slate-100 rounded-full" />
                    </div>
                    <div className="p-6">
                      {/* CPL metric */}
                      <div className="flex justify-between items-end mb-6">
                        <div>
                          <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">
                            Costo por Lead (CPL)
                          </p>
                          <p className="text-3xl font-bold text-slate-900">
                            $68.50{" "}
                            <span className="text-green-500 text-sm font-medium">↓ 38%</span>
                          </p>
                        </div>
                        <div className="w-24 h-12 bg-blue-50 rounded flex items-center justify-center">
                          <svg className="w-16 h-8 text-[#0A66C2]" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 100 40">
                            <path d="M0 5 Q 25 8 40 18 T 70 30 T 100 36" strokeLinecap="round" />
                          </svg>
                        </div>
                      </div>
                      {/* Lead quality */}
                      <div className="mb-5">
                        <div className="flex justify-between text-xs text-slate-400 mb-1">
                          <span>Tasa MQL → SQL</span>
                          <span className="text-green-500 font-semibold">↑ +18pp</span>
                        </div>
                        <p className="text-xl font-bold text-slate-900 mb-2">62%</p>
                        <div className="w-full h-2 bg-slate-100 rounded-full overflow-hidden">
                          <div className="h-2 bg-[#0A66C2] rounded-full" style={{ width: "62%" }} />
                        </div>
                        <div className="flex justify-between text-xs text-slate-400 mt-1">
                          <span>Antes: 44%</span><span>Ahora: 62%</span>
                        </div>
                      </div>
                      {/* Stats row */}
                      <div className="grid grid-cols-3 gap-3 pt-3 border-t border-slate-100">
                        {[
                          { label: "Seniority", value: "C-Level" },
                          { label: "Conv. Rate", value: "8.4%" },
                          { label: "Ad Spend", value: "$12k" },
                        ].map((s) => (
                          <div key={s.label} className="text-center">
                            <p className="text-xs text-slate-400">{s.label}</p>
                            <p className="font-bold text-slate-900 text-sm">{s.value}</p>
                          </div>
                        ))}
                      </div>
                    </div>
                  </div>
                </div>
              </div>

            </div>
          </div>
        </section>

        {/* ── TRUST BAR ── */}
        <section className="border-y border-slate-100 py-8 bg-slate-50">
          <div className="max-w-7xl mx-auto px-4 text-center">
            <p className="text-xs font-bold text-slate-400 uppercase tracking-[0.2em] mb-6">
              Trabajamos con empresas en:
            </p>
            <div className="flex flex-wrap justify-center gap-6 lg:gap-12 items-center opacity-70">
              {[
                { flag: "🇲🇽", label: "México" },
                { flag: "🇨🇴", label: "Colombia" },
                { flag: "🇨🇱", label: "Chile" },
                { flag: "🇦🇷", label: "Argentina" },
                { flag: "🇺🇸", label: "USA" },
              ].map((m) => (
                <div key={m.label} className="flex items-center gap-2 font-semibold text-slate-600">
                  <span>{m.flag}</span> {m.label}
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* ── PROBLEM SECTION (dark) ── */}
        <section className="py-24 bg-[#0F172A] text-white">
          <div className="max-w-4xl mx-auto px-4">
            <h2 className="text-3xl lg:text-4xl font-bold mb-8">
              LinkedIn Ads es caro por definición. Por eso cada peso tiene que ir a la cuenta correcta.
            </h2>
            <div className="space-y-6 text-lg text-slate-300">
              <p>
                El CPL en LinkedIn es estructuralmente más alto que en Google o Meta. No porque
                la plataforma sea ineficiente, sino porque estás pagando por acceso a tomadores
                de decisiones verificados — CEO, VP, Director. Eso vale.
              </p>
              <p>
                El problema es cuando ese presupuesto va a los títulos equivocados, a empresas
                fuera del ICP o a campañas sin tracking conectado al CRM. Entonces pagas como si
                llegaras a decision-makers, pero en realidad no sabes si esos leads avanzan en
                el pipeline.
              </p>
              <p className="font-semibold text-white">
                El diagnóstico empieza por ahí: ¿estás llegando realmente a quien dices que
                llegas, y están convirtiendo en revenue?
              </p>
            </div>
            <div className="mt-10 grid sm:grid-cols-3 gap-6">
              {[
                { stat: "4–6x", label: "más caro que Meta Ads — pero el lead califica mejor" },
                { stat: "62%+", label: "tasa MQL→SQL cuando la segmentación es correcta" },
                { stat: "<4h", label: "tiempo de respuesta para agendar tu diagnóstico" },
              ].map((item) => (
                <div key={item.stat} className="bg-white/5 rounded-xl p-6 border border-white/10">
                  <p className="text-3xl font-black text-[#0A66C2] mb-2">{item.stat}</p>
                  <p className="text-slate-300 text-sm leading-relaxed">{item.label}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* ── WHO IT'S FOR ── */}
        <section className="py-20 bg-slate-50">
          <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-12">
              <h2 className="text-3xl font-bold text-slate-900 mb-4">
                ¿Para quién funciona LinkedIn Ads?
              </h2>
              <p className="text-slate-600 max-w-2xl mx-auto">
                No todas las empresas necesitan LinkedIn Ads. Funciona bien cuando tienes un
                cliente ideal con perfil profesional claro y un ticket promedio que justifica
                el CPL más alto.
              </p>
            </div>
            <div className="grid md:grid-cols-2 gap-6">
              {[
                {
                  emoji: "✅",
                  title: "Funciona muy bien para:",
                  items: [
                    "SaaS B2B con ticket > $500 USD/mes",
                    "Consultoría y servicios enterprise",
                    "Educación ejecutiva y certificaciones",
                    "Software de gestión empresarial (ERP, CRM, HR Tech)",
                    "Servicios financieros y seguros corporativos",
                    "Ciclos de venta de 2+ meses con múltiples stakeholders",
                  ],
                  color: "border-green-200 bg-green-50/50",
                  titleColor: "text-green-700",
                },
                {
                  emoji: "⚠️",
                  title: "Considera otras plataformas si:",
                  items: [
                    "Tu ticket promedio es menor a $200 USD",
                    "Vendes B2C o productos de consumo masivo",
                    "Tu ciclo de venta es menor a 2 semanas",
                    "Tu ICP no tiene título profesional claro",
                    "Tu presupuesto mensual es menor a $5,000 USD",
                    "No tienes proceso de calificación de leads",
                  ],
                  color: "border-amber-200 bg-amber-50/50",
                  titleColor: "text-amber-700",
                },
              ].map((col) => (
                <div key={col.title} className={`rounded-2xl p-8 border ${col.color}`}>
                  <h3 className={`text-lg font-bold mb-4 ${col.titleColor}`}>
                    {col.emoji} {col.title}
                  </h3>
                  <ul className="space-y-2">
                    {col.items.map((item) => (
                      <li key={item} className="text-slate-700 text-sm flex items-start gap-2">
                        <span className="mt-0.5 text-slate-400">—</span>
                        {item}
                      </li>
                    ))}
                  </ul>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* ── SERVICES GRID ── */}
        <section className="py-24 bg-white">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              <h2 className="text-3xl font-bold text-slate-900 mb-4">
                Qué gestionamos en LinkedIn Ads
              </h2>
              <p className="text-slate-600 max-w-2xl mx-auto">
                Cada formato tiene un rol distinto en el funnel B2B. La clave está en usar el
                formato correcto en el momento correcto del ciclo de compra.
              </p>
            </div>
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
              {services.map((s) => (
                <div
                  key={s.title}
                  className="p-8 border border-slate-100 rounded-2xl hover:border-blue-100 hover:bg-blue-50/30 transition-all"
                >
                  <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center text-[#0A66C2] mb-6">
                    {s.icon}
                  </div>
                  <h3 className="text-xl font-bold mb-3 text-slate-900">{s.title}</h3>
                  <p className="text-slate-600 text-sm leading-relaxed">{s.body}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* ── FORM ── */}
        <section className="py-24 bg-[#f6f3f2]" id="diagnostico-form">
          <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-10">
              <div className="inline-flex items-center gap-2 px-3 py-1 bg-[#0A66C2] text-white rounded-sm text-[10px] font-bold uppercase tracking-[0.15em] mb-4">
                Diagnóstico Gratuito
              </div>
              <h2
                className="text-3xl md:text-4xl font-black text-[#1c1b1b] mb-3"
                style={{ fontFamily: "Manrope, sans-serif" }}
              >
                Revisamos tu estrategia de LinkedIn Ads
              </h2>
              <p className="text-[#424656]">
                Segmentación, estructura de campañas, tracking y conexión con CRM — en 60 minutos.
                Te contactamos en menos de 4 horas hábiles.
              </p>
            </div>
            <div className="bg-white rounded-2xl shadow-sm border border-[#c2c6d8]/10 p-2 overflow-hidden">
              <AuditoriaForm />
            </div>
          </div>
        </section>

        {/* ── FAQ ── */}
        <section className="py-24 bg-white">
          <div className="max-w-3xl mx-auto px-4">
            <h2 className="text-3xl font-bold text-center mb-12 text-slate-900">
              Preguntas frecuentes sobre LinkedIn Ads
            </h2>
            <div className="space-y-6">
              {faqItems.map((item) => (
                <div key={item.q} className="border-b border-slate-100 pb-6">
                  <h3 className="text-lg font-bold text-slate-900 mb-3">{item.q}</h3>
                  <p className="text-slate-600 leading-relaxed">{item.a}</p>
                </div>
              ))}
            </div>
          </div>
        </section>

        {/* ── FINAL CTA ── */}
        <section className="py-20 bg-[#0A66C2] text-white text-center">
          <div className="max-w-4xl mx-auto px-4">
            <h2 className="text-3xl lg:text-4xl font-bold mb-6 leading-tight">
              Si estás invirtiendo en LinkedIn Ads sin saber si esos leads avanzan en el pipeline,
              es hora de revisar la estrategia.
            </h2>
            <p className="text-xl text-blue-100 mb-10">
              La sesión de diagnóstico es gratuita. En 60 minutos auditamos segmentación,
              tracking y conexión con CRM. Sin compromiso de contratación.
            </p>
            <a
              href="#diagnostico-form"
              className="inline-block bg-white text-[#0A66C2] px-10 py-4 rounded-xl font-bold text-lg hover:bg-slate-50 transition-colors"
            >
              Solicitar diagnóstico gratuito
            </a>
          </div>
        </section>

      </main>
      <Footer />
    </>
  );
}
